...
### 2. 상태 확인 및 해결 방법
이번 장애 상황은 Physical Volume 의 상태만 missing 상태이고 그외에는 정상인 상태의 경우 입니다.
만약 위와 같이 특수한 상황에서의 장애가 아니라 실제로 metadata에 문제가 있다면 다른 방법을 통하여 복구해야 합니다.
Physical Volume이 누락 되었는지 확인 작업이 필요합니다. 가장 간단한 방법은 pvs 명령어를 사용하여 attributes에 m 표시를 확인하는 방법이 있습니다.
```bash
pvs
```
