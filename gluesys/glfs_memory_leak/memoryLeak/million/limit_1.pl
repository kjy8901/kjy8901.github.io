...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_1`;
`echo "#### limit default case ####" >> /root/IO_RES_1 &`;

system ("sh -c 'dstat -n 3600 >> /root/IO_RES_1_h &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_1_1s &'");

while ($glfsPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_1`;
