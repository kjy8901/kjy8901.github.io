...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_iobufdft`;
`echo "#### limit default case ####" >> /root/IO_RES_iobufdft &`;

system ("sh -c 'dstat -n 3600 >> /root/IO_RES_iobufdft_h &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_iobufdft_1s &'");

while ($glfsPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_iobufdft`;
