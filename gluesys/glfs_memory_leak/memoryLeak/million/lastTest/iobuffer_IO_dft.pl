...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_iobuf_dft`;
`echo "#### limit default case ####" >> /root/IO_RES_iobuf_dft &`;

system ("sh -c 'dstat -n 600 >> /root/IO_RES_iobuf_dft_10m &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_iobuf_dft_1s &'");

while ($millPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_iobuf_dft`;
...
#FIND
`echo "#### find start ####" >> /root/IO_RES_iobuf_dft &`;
