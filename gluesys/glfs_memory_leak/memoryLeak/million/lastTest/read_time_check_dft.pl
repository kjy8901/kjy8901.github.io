...
`date '+%Y-%m-%d %H:%M:%S' >> /root/find_2040_dft`;
`echo "#### START FIND LRU = 131072 ####" >> /root/find_2040_dft &`;
...
chomp $glfsPid;
#FIND
`echo "#### find start ####" >> /root/find_2040_dft &`;
...
while ($findPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/find_2040_dft`;
