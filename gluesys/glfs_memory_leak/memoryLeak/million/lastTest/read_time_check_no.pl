...
`date '+%Y-%m-%d %H:%M:%S' >> /root/find_2040_no`;
`echo "#### START FIND LRU = no ####" >> /root/find_2040_no &`;
...
chomp $glfsPid;
#FIND
`echo "#### find start ####" >> /root/find_2040_no &`;
...
while ($findPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/find_2040_no`;
