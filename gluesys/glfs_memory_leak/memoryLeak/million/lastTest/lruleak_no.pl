...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_lruno`;
`echo "#### limit default case ####" >> /root/IO_RES_lruno &`;

system ("sh -c 'dstat -n 600 >> /root/IO_RES_lruno_10m &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_lruno_1s &'");

while ($millPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_lruno`;
...
#FIND
`echo "#### find start ####" >> /root/IO_RES_lruno &`;
