...
system("sh -c 'mount -t glusterfs -o lru-limit=300 10.10.1.222:EC_VOL /mnt/EC_VOL'");

`date '+%Y-%m-%d %H:%M:%S' >> /root/find_2040_300`;
`echo "#### START FIND LRU = 1 ####" >> /root/find_2040_300 &`;
...
chomp $glfsPid;
#FIND
`echo "#### find start ####" >> /root/find_2040_300 &`;
...
while ($findPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/find_2040_300`;
