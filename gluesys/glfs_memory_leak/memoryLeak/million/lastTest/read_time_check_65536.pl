...
`date '+%Y-%m-%d %H:%M:%S' >> /root/find_2040_65536`;
`echo "#### START FIND LRU = 65536 ####" >> /root/find_2040_65536 &`;
...
chomp $glfsPid;
#FIND
`echo "#### find start ####" >> /root/find_2040_65536 &`;
...
while ($findPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/find_2040_65536`;
