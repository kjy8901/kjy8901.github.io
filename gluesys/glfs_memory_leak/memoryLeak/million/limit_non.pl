...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_non`;
`echo "#### limit default case ####" >> /root/IO_RES_non &`;

system ("sh -c 'dstat -n 3600 >> /root/IO_RES_non_h &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_non_1s &'");

while ($glfsPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_non`;
