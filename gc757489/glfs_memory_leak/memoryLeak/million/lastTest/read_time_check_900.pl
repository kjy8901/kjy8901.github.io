...
system("sh -c 'mount -t glusterfs -o lru-limit=900 10.10.1.222:EC_VOL /mnt/EC_VOL'");

`date '+%Y-%m-%d %H:%M:%S' >> /root/find_2040_900`;
`echo "#### START FIND LRU = 1 ####" >> /root/find_2040_900 &`;
...
chomp $glfsPid;
#FIND
`echo "#### find start ####" >> /root/find_2040_900 &`;
...
while ($findPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/find_2040_900`;
