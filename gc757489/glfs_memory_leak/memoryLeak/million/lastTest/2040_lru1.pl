...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_2040_lru1`;
`echo "#### limit 1 case 2M~4M ####" >> /root/IO_RES_2040_lru1 &`;

system ("sh -c 'dstat -n 600 >> /root/IO_RES_2040_lru1_10m &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_2040_lru1_1s &'");

while ($millPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_2040_lru1`;
...
#FIND
`echo "#### find start ####" >> /root/IO_RES_2040_lru1 &`;
