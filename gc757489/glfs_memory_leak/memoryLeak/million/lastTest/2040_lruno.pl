...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_2040_lruno`;
`echo "#### limit 1 case 2M~4M ####" >> /root/IO_RES_2040_lruno &`;

system ("sh -c 'dstat -n 600 >> /root/IO_RES_2040_lruno_10m &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_2040_lruno_1s &'");

while ($millPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_2040_lruno`;
...
#FIND
`echo "#### find start ####" >> /root/IO_RES_2040_lruno &`;
