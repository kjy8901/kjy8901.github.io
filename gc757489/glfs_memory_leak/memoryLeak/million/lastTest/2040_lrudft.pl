...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_2040_lrudft`;
`echo "#### limit 1 case 2M~4M ####" >> /root/IO_RES_2040_lrudft &`;

system ("sh -c 'dstat -n 600 >> /root/IO_RES_2040_lrudft_10m &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_2040_lrudft_1s &'");

while ($millPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_2040_lrudft`;
...
#FIND
`echo "#### find start ####" >> /root/IO_RES_2040_lrudft &`;
