...
system("sh -c 'mount -t glusterfs -o lru-limit=1310 10.10.1.222:EC_VOL /mnt/EC_VOL'");
...
`date '+%Y-%m-%d %H:%M:%S' >> /root/IO_RES_2040_lru1310`;
`echo "#### limit 1 case 2M~4M ####" >> /root/IO_RES_2040_lru1310 &`;

system ("sh -c 'dstat -n 600 >> /root/IO_RES_2040_lru1310_10m &'");
system ("sh -c 'dstat -n 1 >> /root/IO_RES_2040_lru1310_1s &'");

while ($millPid ne "")
{
    `top -b -p $glfsPid -n 1 |tail -1 >> /root/IO_RES_2040_lru1310`;
...
#FIND
`echo "#### find start ####" >> /root/IO_RES_2040_lru1310 &`;
